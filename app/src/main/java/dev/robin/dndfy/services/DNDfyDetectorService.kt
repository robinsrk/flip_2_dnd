...
import dev.robin.dndfy.MainActivity
import dev.robin.dndfy.R
import dev.robin.dndfy.domain.repository.SettingsRepository
import dev.robin.dndfy.data.repository.SettingsRepositoryImpl
import kotlinx.coroutines.*
import kotlinx.coroutines.flow.*
import kotlinx.coroutines.flow.first

private const val TAG = "DNDfyDetectorService"
private const val SERVICE_NOTIFICATION_ID = 1
private const val STATE_NOTIFICATION_ID = 2
private const val DND_NOTIFICATION_ID = 3
private const val SERVICE_CHANNEL_ID = "DNDfyDetectorChannel"
private const val STATE_CHANNEL_ID = "DNDfyStateChannel"

class DNDfyDetectorService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        Log.d(TAG, "onCreate: Starting DNDfyDetectorService")
    ...
    override fun onDestroy() {
        super.onDestroy()
        Log.d(TAG, "onDestroy: Cleaning up DNDfyDetectorService")
        try {
    ...
    private fun createNotificationChannels() {
        ...
        val stateChannel = NotificationChannel(
            ...
            description = "Notifications about device state and DND changes"
            setSound(null, null)
            enableVibration(false)
            enableLights(false)
        }

        notificationManager.createNotificationChannels(listOf(serviceChannel, stateChannel))
    }

    private fun showDeviceFlippedNotification() {
        ...
        serviceScope.launch {
            ...
            if (!notificationsEnabled) {
                Log.d(TAG, "Notifications disabled in settings - skipping device flip notification")
                return@launch
            }
            
            val notificationManager = getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
            val notification = NotificationCompat.Builder(this@DNDfyDetectorService, STATE_CHANNEL_ID)
                .setContentTitle("Device Flipped")
            ...
            Log.d(TAG, "Device flip notification shown")
        }
    }
